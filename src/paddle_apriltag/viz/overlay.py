...
def draw_paddle_pose(
    ...
    try:
        origin_xy, y_end, x_end = paddle_axis_image_points(
            pose.rotation, pose.origin, camera_matrix, dist_coeffs, marker_size_m * 0.5
        )
        cv2.arrowedLine(frame, origin_xy, y_end, (0, 0, 255), 2, tipLength=0.25)
        cv2.arrowedLine(frame, origin_xy, x_end, (255, 255, 0), 2, tipLength=0.25)
...
def draw_marker_annotations(
    ...
    *,
    ...
    try:
        ...
        _, paddle_y_end, paddle_x_end = paddle_axis_image_points(
            paddle_rotation, paddle_origin, camera_matrix, dist_coeffs, marker_size_m * 0.35
        )
        cv2.arrowedLine(frame, (px, py), paddle_y_end, (255, 0, 255), 2, tipLength=0.25)
        cv2.arrowedLine(frame, (px, py), paddle_x_end, (255, 255, 0), 2, tipLength=0.25)
    except RuntimeError:
        pass
