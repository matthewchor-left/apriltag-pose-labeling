"""Camera calibration using an OpenCV chessboard + marker pattern board."""
...
# OpenCV CharucoBoard uses classic square marker dictionaries (not AprilTag).
CALIBRATION_BOARD_DICTS = {
...
def build_calibration_board(
    ...
    if dictionary_name not in CALIBRATION_BOARD_DICTS:
        raise ValueError(f"Unknown dictionary {dictionary_name!r}.")
    dictionary = cv2.aruco.getPredefinedDictionary(CALIBRATION_BOARD_DICTS[dictionary_name])
...
def board_object_image_points(
    board_corners: np.ndarray,
    board_ids: np.ndarray,
    chessboard_corners_3d: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    corner_ids = board_ids.flatten()
    obj_pts = chessboard_corners_3d[corner_ids].reshape(-1, 1, 3)
    img_pts = np.asarray(board_corners, dtype=np.float32).reshape(-1, 1, 2)
    return obj_pts, img_pts


def normalize_board_detection(
    board_corners: np.ndarray | None,
    board_ids: np.ndarray | None,
) -> tuple[np.ndarray, np.ndarray] | None:
    if board_corners is None or board_ids is None:
        return None
    corners = np.asarray(board_corners, dtype=np.float32).reshape(-1, 2)
    ids = np.asarray(board_ids, dtype=np.int32).reshape(-1)
    if corners.size == 0 or ids.size == 0 or corners.shape[0] != ids.shape[0]:
        return None
    return corners, ids


def draw_board_detection(
    frame: np.ndarray,
    board_corners: np.ndarray | None,
    board_ids: np.ndarray | None,
    ...
    detection = normalize_board_detection(board_corners, board_ids)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Calibrate camera with a printed chessboard pattern.")
    ...
    parser.add_argument("--dictionary", choices=sorted(CALIBRATION_BOARD_DICTS), default="4x4_50")
    ...
    board, board_detector, chessboard_corners_3d = build_calibration_board(
        args.squares_x, args.squares_y, args.square_size, marker_size, args.dictionary,
    )
    inner_corners = board.getChessboardSize()
    print(
        f"Calibration board: {args.squares_x}x{args.squares_y} squares, "
    ...
    print("Show the calibration board from varied angles. Space: capture, q: finish.")
    ...
    while True:
        ...
        board_corners, board_ids, marker_corners, marker_ids = board_detector.detectBoard(gray)
        detection = normalize_board_detection(board_corners, board_ids)
        enough_corners = detection is not None and detection[1].size >= args.min_corners

        draw_board_detection(frame, board_corners, board_ids, marker_corners, marker_ids)
        ...
        if key == ord(" "):
            if enough_corners and detection is not None:
                corners, ids = detection
                obj_pts, img_pts = board_object_image_points(corners, ids, chessboard_corners_3d)
    ...
    if not objpoints:
        raise RuntimeError("No calibration board detections collected.")
    ...
    save_calibration_json(
        args.output,
        {
            "calibration_source": "calibration_board",
